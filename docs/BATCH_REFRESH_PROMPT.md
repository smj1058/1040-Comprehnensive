...
> | 11 | **Notion write — pointer-only model.** The `dossier.md` lives in the repo on the working branch; the Executive Summaries DB row references it via the `📄 Dossier` URL property and does NOT duplicate the body content. Steps: (a) `notion-update-page` `update_properties` on the Executive Summaries DB row to populate Summary (3000-char TL;DR pulled from §1) / Summary Type / Tax Engagement Type / Reporting Entity / date:Last Synthesized:start / 📄 Dossier (GitHub blob URL of dossier.md on the branch) / 📧 / 🎙 / ⚡ / 📁 / 🤖 (each = same blob URL with section anchor). (b) `notion-create-pages` into the Claude Summaries DB to log this refresh (Thread Title, Account relation, Clients, Topics, Files Reviewed, Notion Pages Created, Summary, Date). (c) Read-after-write: `notion-fetch` the row to confirm. (d) Update state.json `turn_11_push_notion: done` + commit + push. **DO NOT** push the dossier markdown into the Notion page body via `replace_content` — it's redundant with the 📄 Dossier URL and creates a drift-prone duplicate. | Notion write |
>
> **Hard rules**
>
